package com.mekanismbiggerteleporter.util;
...
public class BiggerTeleporterUtil {
    ...
    public static double calculateTeleportersDistance(TileEntityTeleporter tile) {
        ...
        Coord4D thisTeleporter = tile.getTileCoord();
        Coord4D targetTeleporter = frequency.getClosestCoords(thisTeleporter);

        if (targetTeleporter == null) {
            return -1;
        }

        if (!thisTeleporter.dimension.equals(targetTeleporter.dimension)) {
            return -1;
        }

        double dx = targetTeleporter.getX() - thisTeleporter.getX();
        double dy = targetTeleporter.getY() - thisTeleporter.getY();
        double dz = targetTeleporter.getZ() - thisTeleporter.getZ();
